...
class Network {
    ...
    internal func request<T: Codable>(_ urlRequest: URLRequest, responseAs: T.Type) -> AnyPublisher<T?, Never> {
    ...
    internal func request<T: Codable>(_ urlRequest: URLRequest, responseAs: T.Type) -> AnyPublisher<T, Error> {
        let request = adapt(urlRequest)
